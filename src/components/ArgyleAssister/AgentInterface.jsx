...
import { Shield, Cpu, Send, X, Mic, MicOff, Volume2, VolumeX, Languages, Sparkles, Settings } from 'lucide-react';
...
const AgentInterface = () => {
    ...
    useEffect(() => {
        const loadVoices = () => {
            ...
            if (!currentVoiceName && voices.length > 0) {
                const auto = voices.find(v => v.name.toLowerCase().includes('natural')) || voices[0];
                setCurrentVoiceName(auto.name);
                VoiceHandler.setVoice(auto.name);
            }
        };
    ...
    useEffect(() => {
        scrollToBottom();
        // Cancel speech if closed or mode changed
        if (!isOpen) VoiceHandler.synthesis?.cancel();
    ...
    const findGeneralResponse = async (query, context = {}) => {
        ...
        const wasTalkingAboutSchedule = lastAgentMessage.toLowerCase().includes('schedule') ||
                                        ...
                                        lastAgentMessage.toLowerCase().includes('end');
